...
BLACKLISTS = {
    # Ideogram 4 is a 34-layer single-stream DiT. Default profile keeps
    # conditioning, timestep/AdaLN, norms, embeddings, and final projection raw,
    # while converting the large attention/MLP 2D weights.
    "Ideogram-4-FP8": [
        "bias", "norm", "adaln", "adaLN", "t_embedding", "t_embed", "time",
        "input_proj", "llm_cond", "embed", "rotary", "rope", "position",
        "final_layer",
    ],
    # A little more compact: also quantizes input/LLM conditioning projections,
    # but still keeps AdaLN, norms, embeddings, and final output raw.
    "Ideogram-4-FP8-Aggressive": [
        "bias", "norm", "adaln", "adaLN", "t_embedding", "t_embed", "time",
        "embed", "rotary", "rope", "position", "final_layer",
    ],
    ...
    with safe_open(str(path), framework="pt", device="cpu") as f:
        ...
        for k in keys:
            ...
            for label, needles in {
                "ideogram_layers": ["layers."],
                "ideogram_attention": ["attention.qkv", "attention.o", "attention"],
                "ideogram_mlp": ["feed_forward.w1", "feed_forward.w2", "feed_forward.w3"],
                "ideogram_conditioning": ["llm_cond", "t_embedding", "adaln", "embed_image_indicator", "input_proj"],
...
def convert(args):
    ...
    log("=== NVFP4 Kitchen Lab v3 ===")
    log("Converter scans key structure, strips stale FP8 quant aux tensors, and can convert 2D weights to NVFP4.")
    ...
    quant_map = {
        ...
        "warning": "Generated by an unofficial NVFP4 converter wrapper using comfy-kitchen TensorCoreNVFP4Layout. Model-sensitive tensors are kept in BF16/FP16 according to the selected profile.",
    }
    ...
    final_metadata["converted_by"] = "NVFP4 Kitchen Lab v3"
...
def main():
    p = argparse.ArgumentParser(description="ComfyUI NVFP4 converter lab using comfy-kitchen TensorCoreNVFP4Layout")
